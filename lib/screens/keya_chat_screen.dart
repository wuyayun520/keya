...
class KeyaChatScreen extends StatefulWidget {
  final String userId;
  final String userName;
  final String userAvatar;

  const KeyaChatScreen({
    super.key,
  ...
  @override
  State<KeyaChatScreen> createState() => _KeyaChatScreenState();
}

class _KeyaChatScreenState extends State<KeyaChatScreen> {
  List<_KeyaChatMessage> _messages = [];
  ...
  Future<String> _getMediaDirectory() async {
    ...
    final mediaDir = Directory('${dir.path}/keya_chat_media');
  ...
  Future<void> _loadMessages() async {
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/keya_chat_history_${widget.userId}.json');
    if (await file.exists()) {
      try {
        ...
        setState(() {
          _messages = jsonList.map((e) => _KeyaChatMessage.fromJson(e)).toList();
        });
  ...
  void _sendMessage() {
    ...
    setState(() {
        _messages.add(_KeyaChatMessage(
        text: text,
        isMe: true,
        time: _getTimeStamp(),
          type: _KeyaChatMessageType.text,
      ));
    });
  ...
  Future<void> _sendImage(File imageFile) async {
    try {
      ...
      final fileName = 'keya_image_${DateTime.now().millisecondsSinceEpoch}.jpg';
      ...
      setState(() {
        _messages.add(_KeyaChatMessage(
          ...
          isMe: true,
          time: _getTimeStamp(),
          type: _KeyaChatMessageType.image,
        ));
      });
      _scrollToBottom();
      _saveMessages();
  ...
  Future<void> _sendVoice(String audioPath, Duration duration) async {
    try {
      final mediaDir = await _getMediaDirectory();
      final fileName = 'keya_voice_${DateTime.now().millisecondsSinceEpoch}.m4a';
      ...
      setState(() {
        _messages.add(_KeyaChatMessage(
          ...
          isMe: true,
          time: _getTimeStamp(),
          type: _KeyaChatMessageType.audio,
        ));
      });
      _scrollToBottom();
      _saveMessages();
    ...
    try {
      if (_isRecording) {
        ...
        if (await _voiceRecorder.hasPermission()) {
          ...
          final filePath = '${dir.path}/keya_voice_${DateTime.now().millisecondsSinceEpoch}.m4a';
  ...
  void _showInfoSnackBar(String message) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        backgroundColor: const Color(0xFF9355FF).withOpacity(0.8),
        duration: const Duration(seconds: 2),
        behavior: SnackBarBehavior.floating,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
        margin: const EdgeInsets.all(16),
...
enum _KeyaChatMessageType { text, image, audio }

class _KeyaChatMessage {
  final String? text;
  ...
  final bool isMe;
  final String time;
  final _KeyaChatMessageType type;
  ...
  static _KeyaChatMessage fromJson(Map<String, dynamic> json) => _KeyaChatMessage(
    text: json['text'],
    ...
    isMe: json['isMe'] ?? true,
    time: json['time'] ?? '',
    type: _KeyaChatMessageType.values.firstWhere(
      (e) => e.name == json['type'],
      orElse: () => _KeyaChatMessageType.text,
    ),
  );
}

class _KeyaChatBubble extends StatefulWidget {
  final _KeyaChatMessage message;
  final void Function(File file)? onImageTap;
  const _KeyaChatBubble({required this.message, this.onImageTap});

  @override
  State<_KeyaChatBubble> createState() => _KeyaChatBubbleState();
}

class _KeyaChatBubbleState extends State<_KeyaChatBubble> {
  ...
  void initState() {
    super.initState();
    if (widget.message.type == _KeyaChatMessageType.audio) {
  ...
  Future<void> _togglePlayPause() async {
    try {
      ...
      final dir = await getApplicationDocumentsDirectory();
      final absPath = '${dir.path}/keya_chat_media/${msg.audioPath}';
  ...
  Widget build(BuildContext context) {
    ...
    if (msg.type == _KeyaChatMessageType.audio && msg.audioPath != null) {
    ...
    if (msg.type == _KeyaChatMessageType.image && msg.imagePath != null) {
      return Align(
        alignment: msg.isMe ? Alignment.centerRight : Alignment.centerLeft,
        child: GestureDetector(
          onTap: () async {
            final dir = await getApplicationDocumentsDirectory();
            final absPath = '${dir.path}/keya_chat_media/${msg.imagePath}';
                ...
                ClipRRect(
                  ...
                  child: FutureBuilder<Directory>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      final absPath = '${snapshot.data!.path}/keya_chat_media/${msg.imagePath}';
                      return Image.file(
                        File(absPath),
                        width: 160,
                        height: 160,
...
class _KeyaChatInputBar extends StatelessWidget {
