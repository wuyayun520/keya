...
fileprivate let notiFatalMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
// __CLOSE_PRINT__
//
//  ConsultativeDurationCoord.swift
//  AbroadTalking
//
...
let main_revenueMessage = 80.0
//: let kProgressHUD_cornerRadius = 14.0
let mainPackagePath = 14.0
//: let kProgressHUD_alpha        = 0.9
let data_trustStr = 0.9
//: let kBackgroundView_alpha     = 0.6
let showReplaceFormat = 0.6
//: let kAnimationInterval        = 0.2
let user_methodStr = 0.2
//: let kTransformScale           = 0.9
let mainInstallText = 0.9

//: open class ProgressHUD: UIView {
open class ConsultativeDurationCoord: UIView {
    //: required public init?(coder: NSCoder) {
    public required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: notiFatalMessage.reversed(), encoding: .utf8)!)
    }

    //: static var shared = ProgressHUD()
    static var shared = ConsultativeDurationCoord()
    ...
    class func post(superView: UIView?) {
        //: if superView != nil {
        if superView != nil {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                //: ProgressHUD.shared.frame = superView!.bounds
                ConsultativeDurationCoord.shared.frame = superView!.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                ConsultativeDurationCoord.shared.activityIndicator.center = ConsultativeDurationCoord.shared.center
                //: superView!.addSubview(ProgressHUD.shared)
                superView!.addSubview(ConsultativeDurationCoord.shared)
            }
            //: } else {
        } else {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                ...
                ConsultativeDurationCoord.shared.frame = UIScreen.main.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                ConsultativeDurationCoord.shared.activityIndicator.center = ConsultativeDurationCoord.shared.center
                //: AppConfig.getWindow().addSubview(ProgressHUD.shared)
                NavigationAtAction.makeApp().addSubview(ConsultativeDurationCoord.shared)
            }
        }
        //: ProgressHUD.shared.hud_startAnimating()
        ConsultativeDurationCoord.shared.observer()
    }

    //: class func dismiss() {
    class func query() {
        //: ProgressHUD.shared.hud_stopAnimating()
        ConsultativeDurationCoord.shared.rootAnimating()
    }
    ...
    private func observer() {
        ...
        DispatchQueue.main.async {
            ...
            self.activityIndicator.transform = CGAffineTransform(scaleX: mainInstallText, y: mainInstallText)
            //: self.activityIndicator.alpha = 0
            self.activityIndicator.alpha = 0
            //: UIView.animate(withDuration: kAnimationInterval) {
            UIView.animate(withDuration: user_methodStr) {
                //: self.backgroundColor = UIColor(white: 0, alpha: kBackgroundView_alpha)
                self.backgroundColor = UIColor(white: 0, alpha: showReplaceFormat)
                //: self.activityIndicator.transform = CGAffineTransform(scaleX: 1, y: 1)
                self.activityIndicator.transform = CGAffineTransform(scaleX: 1, y: 1)
                //: self.activityIndicator.alpha = kProgressHUD_alpha
                self.activityIndicator.alpha = data_trustStr
    ...
    private func rootAnimating() {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: user_methodStr) {
                ...
                ConsultativeDurationCoord.shared.removeFromSuperview()
            }
        }
    }
    ...
    private lazy var activityIndicator: UIActivityIndicatorView = {
        ...
        indicator.bounds = CGRect(x: 0, y: 0, width: main_revenueMessage, height: main_revenueMessage)
        //: indicator.center = self.center
        indicator.center = self.center
        //: indicator.backgroundColor = .black
        indicator.backgroundColor = .black
        //: indicator.layer.cornerRadius = kProgressHUD_cornerRadius
        indicator.layer.cornerRadius = mainPackagePath
...
extension ConsultativeDurationCoord {
    //: class func toast(_ str: String?) {
    class func toteUp(_ str: String?) {
        //: toast(str, showTime: 1)
        action(str, showTime: 1)
    }

    //: class func toast(_ str: String?, showTime: CGFloat) {
    class func action(_ str: String?, showTime: CGFloat) {
        ...
        NavigationAtAction.makeApp().addSubview(titleLab)
        ...
        titleLab.center = NavigationAtAction.makeApp().center
